...
@DataJpaTest
@Import({TestcontainersConfiguration.class, DirectingHandler.class, BotProperties.class})
class DirectingHandlerTest {
    
    @Autowired
    DirectingHandler directingHandler;
    ...
    public void registersForCorrectCommandAndStates(){
        assertThat(directingHandler.forCommand()).isEqualTo(Command.DIRECTING);
        assertThat(directingHandler.forStates()).containsExactlyInAnyOrder(UserState.DIRECTING_GET_USERNAME);
    }
    ...
    @Test
    public void directingWithoutUsernameGetsAskedForUsername(){
        long chatId = 2345L;
        ...
        assertThat(texts).isNotNull()
            ...
            .element(0)
            .asInstanceOf(InstanceOfAssertFactories.STRING)
            .contains("Wessen Werke möchtest du begutachten?");
        Mockito.verifyNoInteractions(userRepository, tapeRepository);
        
        assertThat(userStateEntity)
            .extracting(UserStateEntity::getUserState)
            .isEqualTo(UserState.DIRECTING_GET_USERNAME);
    }
    
    @Test
    public void directingWithUnknownUsernameGetsAskedAgain() {
        long chatId = 4567L;
        var userEntity = userRepository.save(UserEntity.builder()
            .username("admin")
            .pin("1234")
